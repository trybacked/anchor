...
export function listEntities(model: SemanticModel): EntitySummary[] {
  return model.entities.map((entity) =>
    validateModelPayload(EntitySummarySchema, entitySummaryFields(entity)),
  );
}
  ...
  const relations =
    entityId === undefined
      ...
      : model.relations.filter(
          (relation) => relation.fromEntity === entityId || relation.toEntity === entityId,
        );
