...
import { capReviewQuestions, reviewBudgetDoubts, selectReviewQuestions, } from "../../src/questions.js";
...
describe("capReviewQuestions", () => {
    it("keeps at most MAX_REVIEW_QUESTIONS sorted by risk", () => {
        const entities = Array.from({ length: 15 }, (_, index) => entity(`entity-${String(index)}`, 0.5 + index * 0.01));
    ...
    it("turns dropped questions into explicit budget doubts", () => {
        const entities = Array.from({ length: 15 }, (_, index) => entity(`entity-${String(index)}`, 0.5));
        const allQuestions = selectReviewQuestions(entities, [], [], []);
