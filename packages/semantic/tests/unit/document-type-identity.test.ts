...
import { documentTypeStableKey, stableKeyFromEntity } from "../../src/document-type-identity.js";
...
describe("document type stable identity", () => {
  it("uses documentType slug for entity ids regardless of header fingerprint", () => {
    ...
    const coldEntity = buildDocumentCorpusEntities(
      coldCatalog,
      profileForCatalog(coldCatalog),
    ).find((entity) => entity.sourceTable === documentTypeTableName("publication"));
    const incrementalEntity = buildDocumentCorpusEntities(
      incrementalCatalog,
      profileForCatalog(incrementalCatalog),
    ).find((entity) => entity.sourceTable === documentTypeTableName("publication"));
    expect(coldEntity?.id).toBe("dtype_publication");
    expect(incrementalEntity?.id).toBe("dtype_publication");
    expect(coldCatalog.documents[0]?.headerContentFingerprint).not.toBe(
      incrementalCatalog.documents[0]?.headerContentFingerprint,
    );
  });
  ...
  it("maps legacy fingerprint ids back to document type via source table during merge", () => {
    ...
    const freshEntity = buildDocumentCorpusEntities(catalog, profile).find(
      (entity) => entity.sourceTable === documentTypeTableName("publication"),
    )!;
