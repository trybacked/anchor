...
describe("extractFactsFromLine", () => {
  it("labels each quantity with the nearest preceding cue", () => {
    const facts = factsFor(
      "Importo contrattuale € 3.899.802,03 con ribasso del 30,505% e oneri per la sicurezza € 167.920,53",
      PROCUREMENT_VOCABULARY,
      "Affidamento a EDIL VINCENT SRL, CIG Z123456789",
    );
    expect(amountsByType(facts)).toEqual(
      new Map([
  ...
  it("attaches the nearest entity and coded identifier on the page", () => {
    const facts = factsFor(
      "Importo contrattuale € 3.899.802,03",
      PROCUREMENT_VOCABULARY,
      "Affidamento a EDIL VINCENT SRL, CIG Z123456789",
    );
  ...
  it("gives the default fact type to a quantity that matches no cue", () => {
    const facts = extractFactsFromLine(
      line("Totale complessivo € 1.000,00"),
      [],
      PROCUREMENT_VOCABULARY,
    );
...
describe("extractFactsFromLine across vocabularies", () => {
  it("reads the same shapes from an unrelated corpus and locale", () => {
    const facts = factsFor(
      "Invoice total $1,250.00 after a discount of 12.5%, shipping $75.00",
      ENGLISH_INVOICE_VOCABULARY,
      "Issued by ACME Building Inc under PO 4451",
    );
    expect(amountsByType(facts)).toEqual(
      new Map([
        ["invoice_total", 1250],
...
describe("extractFactsFromLines", () => {
  ...
  it("keeps facts attributed to the entity mentioned nearest on the page", () => {
    ...
    expect(mentions.filter((mention) => mention.mentionType === ENTITY_MENTION_TYPE)).toHaveLength(
      2,
    );
    ...
    const rows = [
      {
        ...
        page: 1,
        line: 990,
        text: "Affidamento a EDIL VINCENT SRL, CIG Z1234567890",
      },
      {
        document_id: "doc_1",
        page: 2,
        line: 3,
        text: "Impegno di spesa per lavori per €39.315,88 oltre IVA",
      },
    ];
    const mentions = extractMentionsFromLines(rows, PROCUREMENT_VOCABULARY);
    const facts = extractFactsFromLines(rows, mentions, PROCUREMENT_VOCABULARY);
...
describe("Italian PA document fixture", () => {
  it("extracts a rich fact set from realistic determination lines", () => {
    const mentions = extractMentionsFromLines(
      ITALIAN_PA_DETERMINATION_LINES,
      PROCUREMENT_VOCABULARY,
    );
    const facts = extractFactsFromLines(
      ITALIAN_PA_DETERMINATION_LINES,
      mentions,
      PROCUREMENT_VOCABULARY,
    );
    ...
    expect(
      facts.some((fact) => fact.factType === "importo_contrattuale" && fact.amount === 3899802.03),
    ).toBe(true);
    expect(
      facts.some((fact) => fact.factType === "oneri_sicurezza" && fact.amount === 167920.53),
    ).toBe(true);
    expect(facts.some((fact) => fact.factType === "ribasso_pct" && fact.amount === 30.505)).toBe(
      true,
    );
    expect(
      facts.some((fact) => fact.factType === "impegno_spesa" && fact.amount === 39315.88),
    ).toBe(true);
    expect(facts.some((fact) => fact.factType === "liquidazione" && fact.amount === 45678.9)).toBe(
      true,
    );
    expect(facts.some((fact) => fact.factType === "liquidazione" && fact.amount === 8000)).toBe(
      true,
    );
    expect(facts.some((fact) => fact.rawText === "12.500,00 euro" && fact.amount === 12500)).toBe(
      true,
    );
...
describe("Italian PA currency notation", () => {
  it("reads Euro and euro marks alongside the euro symbol", () => {
    const euroPrefix = extractFactsFromLine(
      line("Liquidazione fattura per Euro 45.678,90"),
      [],
      PROCUREMENT_VOCABULARY,
    );
    const euroSuffix = extractFactsFromLine(
      line("Spesa sostenuta pari a 12.500,00 euro"),
      [],
      PROCUREMENT_VOCABULARY,
    );
    const euroTight = extractFactsFromLine(
      line("Impegno di spesa per lavori per €39.315,88 oltre IVA"),
      [],
      PROCUREMENT_VOCABULARY,
    );
