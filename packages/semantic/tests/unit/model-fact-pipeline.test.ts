...
function extractFactsLikeModelCommand(lineRows: typeof ITALIAN_PA_DETERMINATION_LINES, vocabulary: typeof ITALIAN_PROCUREMENT_VOCABULARY) {
...
describe("model CLI fact pipeline", () => {
    it("extracts seven facts from the Italian PA fixture through the model path", () => {
        const { facts } = extractFactsLikeModelCommand(ITALIAN_PA_DETERMINATION_LINES, ITALIAN_PROCUREMENT_VOCABULARY);
    ...
    it("still extracts currency when domain discovery omits currency fact types", () => {
        const vocabularyWithoutCurrency = {
            ...
            factTypes: ITALIAN_PROCUREMENT_VOCABULARY.factTypes.filter((factType) => factType.quantity !== "currency"),
        };
