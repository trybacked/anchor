...
function extractFactsLikeModelCommand(
  lineRows: typeof ITALIAN_PA_DETERMINATION_LINES,
  vocabulary: typeof PROCUREMENT_VOCABULARY,
) {
...
describe("model CLI fact pipeline", () => {
  it("extracts seven facts from the Italian PA fixture through the model path", () => {
    const { facts } = extractFactsLikeModelCommand(
  ...
  it("still extracts currency when domain discovery omits currency fact types", () => {
    const vocabularyWithoutCurrency = {
      ...PROCUREMENT_VOCABULARY,
      factTypes: PROCUREMENT_VOCABULARY.factTypes.filter(
        (factType) => factType.quantity !== "currency",
      ),
    };
    const { facts, vocabulary } = extractFactsLikeModelCommand(
      ITALIAN_PA_DETERMINATION_LINES,
      vocabularyWithoutCurrency,
    );
