...
const mixedProfile: ProfileReport = [
  ...
  tableProfile("document_chunks", ["document_id", "chunk_index", "page_start", "page_end", "text"]),
];
...
describe("proposeModel mixed folder", () => {
  it("returns structured and document-type entities when a document catalog is present", async () => {
    ...
    expect(
      proposal.entities.some(
        (entity) => entity.sourceTable === noticeTable && entity.id.startsWith("dtype_"),
      ),
    ).toBe(true);
    expect(
      proposal.doubts.some(
        (doubt) =>
          doubt.topic.includes("entity notice") &&
          doubt.reason.includes("duplicate document entity"),
      ),
    ).toBe(true);
  ...
  it("does not add mention entities when mention tables are absent", async () => {
    ...
    expect(proposal.entities.some((entity) => entity.id === "organization")).toBe(false);
    expect(proposal.entities.some((entity) => entity.id === "document_mention")).toBe(false);
  });
