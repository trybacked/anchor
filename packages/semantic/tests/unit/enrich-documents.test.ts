...
describe("buildDocumentTopicSample", () => {
  it("drops the corpus's recurring lines and keeps document-specific ones", () => {
    const boilerplate = findBoilerplateLines(corpusRows());
    const sample = buildDocumentTopicSample(
      [LETTERHEAD, "Oggetto: appalto strada finanziato dal PNRR", FOOTER],
      boilerplate,
    );
  ...
  it("keeps every substantive line when nothing recurs", () => {
    const sample = buildDocumentTopicSample(
      ["   ", "AVVISO DI CONVOCAZIONE", "Si rende noto"],
      new Set(),
    );
  ...
  it("caps the excerpt length", () => {
    const sample = buildDocumentTopicSample(
      Array.from(
        { length: 40 },
        () => "lavori di manutenzione straordinaria della strada comunale",
      ),
      new Set(),
    );
...
describe("enrichDocuments", () => {
  ...
  it("builds the label space and prompt from the vocabulary", async () => {
    ...
    expect(() =>
      request?.schema.parse({
        documents: [{ documentId: "doc_a", topics: ["hotel"], summary: "" }],
      }),
    ).toThrow();
  });
  ...
  it("splits large corpora into batches and sums usage", async () => {
    ...
    const documents = Array.from({ length: DOCUMENT_ENRICHMENT_BATCH_SIZE + 1 }, (_, index) =>
      documentEntry(`doc_${String(index)}`, "Oggetto"),
    );
    const result = await enrichDocuments({
      model: {} as never,
      documents,
