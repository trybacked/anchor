import {
  REJECTED_NAME_SUFFIXES,
  SUFFIX_MAX_LENGTH,
  SUFFIX_MAX_RESULTS,
  SUFFIX_MIN_DOCUMENTS,
  SUFFIX_MIN_LENGTH,
} from "./constants.js";
import type { DocumentLineRow } from "./extract-document-mentions.js";
const DOTTED_SUFFIX_PATTERN = /\s((?:[\p{Lu}]\.\s*){2,}[\p{Lu}]\.?)/gu;
const CAPS_SUFFIX_PATTERN =
  /\b(?:[A-Z]{2,}(?:\s+[A-Z]{2,}){0,5})\s+([A-Z]{3,4})\b(?!\.\s*[\p{Lu}])/gu;
