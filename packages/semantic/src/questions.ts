...
function entityImpact(entity: Entity, relations: Relation[], rules: Rule[]): number {
    const dependentRelations = relations.filter((relation) => relation.fromEntity === entity.id || relation.toEntity === entity.id).length;
    const dependentRules = rules.filter((rule) => rule.appliesTo === entity.id).length;
    return 1 + dependentRelations + dependentRules;
}
function buildQuestion(kind: ReviewQuestion["kind"], targetId: string, question: string, impact: number, confidence: number, evidence: EvidenceTable): QuestionCandidate {
...
export function selectReviewQuestions(entities: Entity[], relations: Relation[], rules: Rule[], tables: CompressedTable[], reviewConfidenceThreshold: number = DEFAULT_REVIEW_CONFIDENCE_THRESHOLD): ReviewQuestion[] {
    ...
    const candidates: QuestionCandidate[] = [
        ...entities
            ...
            .map((entity) => buildQuestion("entity", entity.id, `Does table "${entity.sourceTable}" represent the entity "${entity.name}"?`, entityImpact(entity, relations, rules), entity.confidence, entityEvidence(entity, tables))),
        ...relations
            .filter((relation) => needsReview(relation.confidence))
            .map((relation) => buildQuestion("relation", relation.id, `Confirm relation "${relation.name}" (${relation.fromEntity}.${relation.fromColumn} → ${relation.toEntity}.${relation.toColumn})?`, 2, relation.confidence, relationEvidence(relation, tables))),
        ...rules
            .filter((rule) => needsReview(rule.confidence))
            .map((rule) => buildQuestion("rule", rule.id, `Confirm definition "${rule.name}": ${rule.definition}`, 1, rule.confidence, ruleEvidence(rule, tables))),
    ];
    return candidates
...
export function capReviewQuestions(questions: ReviewQuestion[], maxQuestions: number = MAX_REVIEW_QUESTIONS): CappedReviewQuestions {
