...
import { resolveClassificationBatchSize, resolveSemanticRequestTimeoutMs } from "./env.js";
...
export async function proposeModel(options: ProposeModelOptions): Promise<Proposal> {
  ...
  const allQuestions = buildReviewQuestions(assembly, routing, documentCatalog, vocabulary);
