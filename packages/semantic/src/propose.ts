import { EMPTY_DOMAIN_VOCABULARY, LOW_CONFIDENCE_THRESHOLD, ProposalSchema, } from "@backed/core";
...
import { resolveClassificationBatchSize, resolveReviewConfidenceThreshold, resolveSemanticRequestTimeoutMs, } from "./env.js";
...
import { COLUMN_CLASSIFICATION_SYSTEM_PROMPT, ONTOLOGY_SYSTEM_PROMPT, columnClassificationPrompt, ontologyPrompt, } from "./prompts.js";
...
import { buildDocumentCorpusEntities, buildDocumentCorpusRelations, classifyMentionTables, classifyTypedDocumentTables, materializedEntityIds, } from "./document-ontology.js";
import { slugify } from "./string-utils.js";
import { buildLineDocumentEntities, classifyLineDocumentTables, classifyPipelineMetadataTables, isDocumentCorpus, } from "./line-document.js";
import { formatRoutingSummary, isDocumentPipelineTable, routeTables, type TableRouting, } from "./table-routing.js";
...
function classificationLookup(output: ColumnClassificationOutput): Map<string, Map<string, ColumnClassification>> {
...
function buildProperty(table: TableProfile, column: ColumnProfile, classification: ColumnClassification | undefined): Property {
    return {
...
function assembleEntities(ontology: OntologyOutput, profile: ProfileReport, classification: ColumnClassificationOutput, doubts: Doubt[]): Entity[] {
    ...
    for (const candidate of ontology.entities) {
        ...
        if (!table) {
            doubts.push(droppedDoubt(`entity ${candidate.id}`, `Proposed entity "${candidate.name}" references table "${candidate.sourceTable}", which does not exist in the profile.`, "Proposal dropped: unknown source table."));
            continue;
        }
        ...
        entities.push({
            ...
            properties: table.columns.map((column) => buildProperty(table, column, columnClassifications?.get(column.name))),
        });
    }
    return entities;
}
function assembleRelations(ontology: OntologyOutput, entities: Entity[], doubts: Doubt[]): Relation[] {
    ...
    for (const candidate of ontology.relations) {
        ...
        const fromColumnExists = fromEntity?.properties.some((property) => property.columnName === candidate.fromColumn);
        const toColumnExists = toEntity?.properties.some((property) => property.columnName === candidate.toColumn);
        if (!fromEntity || !toEntity || !fromColumnExists || !toColumnExists) {
            doubts.push(droppedDoubt(`relation ${candidate.id}`, `Proposed relation "${candidate.name}" references entities or columns not present in the profile.`, "Proposal dropped: unverifiable references."));
            continue;
        }
        relations.push({
            id: candidate.id,
...
function assembleRules(ontology: OntologyOutput, entities: Entity[], doubts: Doubt[]): Rule[] {
    ...
    for (const candidate of ontology.rules) {
        ...
        if (!entity) {
            doubts.push(droppedDoubt(`rule ${candidate.id}`, `Proposed rule "${candidate.name}" applies to entity "${candidate.appliesTo}", which does not exist.`, "Proposal dropped: unknown reference entity."));
            continue;
        }
        rules.push({
            id: candidate.id,
...
function documentEntityIds(catalog: DocumentCatalog, vocabulary: DomainVocabulary): Set<string> {
...
function mergeClassificationOutputs(...outputs: ColumnClassificationOutput[]): ColumnClassificationOutput {
...
function filterDocumentEntitiesFromOntology(ontology: OntologyOutput): {
    ...
    const entities = ontology.entities.filter((candidate) => {
        ...
        doubts.push(droppedDoubt(`entity ${candidate.id}`, `LLM proposed document entity "${candidate.name}" on table "${candidate.sourceTable}" — document entities are built deterministically.`, "Proposal dropped: duplicate document entity from LLM."));
        return false;
    });
    return { ontology: { ...ontology, entities }, doubts };
}
function classificationForTables(classification: ColumnClassificationOutput, tables: CompressedTable[]): ColumnClassificationOutput {
...
function resolveOntologyStrategy(routing: TableRouting, documentCatalog: DocumentCatalog | undefined): OntologyStrategy {
    const documentCorpus = documentCatalog !== undefined ||
...
async function classifyAllColumns(routing: TableRouting, documentCatalog: DocumentCatalog | undefined, vocabulary: DomainVocabulary, profile: ProfileReport, models: SemanticModels, batchSize: number, timeoutMs: number, onProgress?: (message: string) => void, onBatchProgress?: (progress: {
    ...
    const lineDocumentClassification = documentCatalog !== undefined
        ? emptyClassification()
        : classifyLineDocumentTables(routing.lineDocuments);
    const typedDocumentClassification = documentCatalog !== undefined ? classifyTypedDocumentTables(documentCatalog) : emptyClassification();
    const metadataClassification = classifyPipelineMetadataTables(routing.pipelineMetadata);
    const mentionClassification = documentCatalog !== undefined
...
function assembleProposal(ontologyOutput: OntologyOutput, ontologyExtraDoubts: Doubt[], classification: ColumnClassificationOutput, profile: ProfileReport, routing: TableRouting, documentCatalog: DocumentCatalog | undefined, vocabulary: DomainVocabulary): AssemblyResult {
    const documentCorpus = documentCatalog !== undefined ||
        isDocumentCorpus(routing.lineDocuments.length, routing.totalTableCount);
    ...
    const lineDocumentEntities = documentCatalog !== undefined
    ...
    const documentRelations = documentCatalog !== undefined
...
function buildReviewQuestions(assembly: AssemblyResult, routing: TableRouting, documentCatalog: DocumentCatalog | undefined, vocabulary: DomainVocabulary, reviewConfidenceThreshold: number): Proposal["questions"] {
    const excludedEntityIds = documentCatalog !== undefined
        ? documentEntityIds(documentCatalog, vocabulary)
        : new Set<string>();
    const standardQuestions = selectReviewQuestions(assembly.entities.filter((entity) => !excludedEntityIds.has(entity.id)), assembly.relations, assembly.rules, routing.allTables, reviewConfidenceThreshold);
    const documentTypeQuestions = documentCatalog !== undefined
        ? selectDocumentTypeReviewQuestions(documentCatalog, assembly.entities, routing.allTables, reviewConfidenceThreshold)
        : [];
    return [...documentTypeQuestions, ...standardQuestions].sort((a, b) => b.risk - a.risk || a.id.localeCompare(b.id));
}
function finalizeReviewQuestions(assembly: AssemblyResult, allQuestions: Proposal["questions"]): Proposal["questions"] {
...
export async function proposeModel(options: ProposeModelOptions): Promise<Proposal> {
    ...
    const reviewConfidenceThreshold = options.reviewConfidenceThreshold ?? resolveReviewConfidenceThreshold();
