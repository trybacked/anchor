...
/** Extract gateway-reported USD cost from provider metadata when present. */
export function extractGatewayCostUsd(providerMetadata: unknown): number | null {
...
function parseWithRecovery<TSchema extends z.ZodTypeAny>(
  schema: TSchema,
...
async function runRawAttempt<TSchema extends z.ZodTypeAny>(
  ...
  const maxOutputTokens =
    RAW_FALLBACK_MAX_OUTPUT_TOKENS[request.schemaName] ?? RAW_FALLBACK_DEFAULT_MAX_OUTPUT_TOKENS;
