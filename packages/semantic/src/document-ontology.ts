...
import {
  ONTOLOGY_ENTITY_CONFIDENCE,
  ONTOLOGY_HIGH_CONFIDENCE,
  ONTOLOGY_PROFILE_CONFIDENCE,
} from "./constants.js";
import {
...
function deriveDocumentTypeColumns(
...
function buildMaterializedEntity(
  spec: MaterializedEntitySpec,
  profile: ProfileReport,
): Entity | null {
...
function mentionEntitySpecs(
  profile: ProfileReport,
  vocabulary: DomainVocabulary,
): MaterializedEntitySpec[] {
  ...
  return [
    {
      ...
      evidence: (rowCount) =>
        `Entity table with ${String(rowCount)} deduplicated ${plural} from body-text extraction`,
    ...
    },
    {
      ...
      evidence: (rowCount) =>
        `Mention table with ${String(rowCount)} extracted mentions from document lines`,
      nullableColumns: ["entity_id"],
    },
    {
      entityId: DOCUMENT_FACT_ENTITY_ID,
      name: "Document Fact",
      description: `Quantities stated in the documents${
        vocabulary.factTypes.length > 0
      ...
      evidence: (rowCount) =>
        `Fact table with ${String(rowCount)} quantities extracted from document lines`,
    ...
    },
    {
      ...
      evidence: (rowCount) =>
        `Rollup with ${String(rowCount)} rows aggregating mentions, facts, and co-occurring ${plural}`,
...
export function classifyMentionTables(
  profile: ProfileReport,
  vocabulary: DomainVocabulary,
): ColumnClassificationOutput {
...
export function buildMentionRelations(
  catalog: DocumentCatalog,
  entities: Entity[],
  vocabulary: DomainVocabulary,
): Relation[] {
...
export function buildDocumentCorpusEntities(
  ...
  return [
    ...
    buildTableEntity(
      DOCUMENT_TEXT_ENTITY_ID,
      "Document Text",
      "Line-level text content for all documents in the corpus",
      DOCUMENT_LINES_TABLE,
      DOCUMENT_LINES_COLUMNS,
      `Unified line table with ${String(lineTable?.rowCount ?? 0)} rows across the document corpus`,
      profile,
    ),
    buildTableEntity(
      DOCUMENT_CHUNK_ENTITY_ID,
      "Document Chunk",
      "Searchable text segments from documents, split for semantic retrieval",
      DOCUMENT_CHUNKS_TABLE,
      DOCUMENT_CHUNK_COLUMNS,
      `Chunk table with ${String(chunkTable?.rowCount ?? 0)} searchable segments across the document corpus`,
      profile,
    ),
...
export function isMaterializedDocumentTable(tableName: string, catalog: DocumentCatalog): boolean {
  return (
    PIPELINE_TABLES.includes(tableName) ||
