...
import {
  ...
  DOCUMENT_EXTRACTION_LLM_SKIP_CONFIDENCE,
} from "./constants.js";
...
import {
  extractVocabularyFields,
  filterHeaderLinesForLlm,
  findRecurringLines,
} from "./extract-header-fields.js";
...
async function extractDocumentBatchWithLlm(
  samples: DocumentExtractionSample[],
  options: {
    models: SemanticModels;
    timeoutMs: number;
...
async function extractDocumentBatchWithLlmOnce(
  samples: DocumentExtractionSample[],
  options: {
    models: SemanticModels;
    timeoutMs: number;
  ...
  const schema =
    samples.length === 1 ? SingleDocumentExtractionSchema : DocumentExtractionOutputSchema;
  ...
  const bySourceTable = new Map(
    output.documents.map((document) => [document.sourceTable, document]),
  );
  const entries: DocumentCatalogEntry[] = [];
  for (const sample of samples) {
...
async function resolveVocabulary(
  vocabulary: DomainVocabulary | Promise<DomainVocabulary> | undefined,
...
export async function extractDocumentCatalog(options: ExtractDocumentCatalogOptions): Promise<{
  ...
  const registryEntries = registrySamples.map(({ sample, registryType }) =>
    buildDeterministicCatalogEntry(
      sample,
      {
  ...
  const deterministicEntries = deterministicSamples.map(({ sample, typeHint }) =>
    buildDeterministicCatalogEntry(
      sample,
      typeHint,
      vocabulary,
