...
import {
  ENTITY_MENTION_CONFIDENCE,
  IDENTIFIER_CHARSET_PATTERNS,
  IDENTIFIER_MENTION_CONFIDENCE,
  MENTION_ENTITY_ID_MAX_LENGTH,
  MENTION_ID_MAX_LENGTH,
  MENTION_MAX_NAME_WORDS,
  MENTION_MIN_NAME_CHARS,
  MENTION_MIN_WORD_CHARS,
} from "./constants.js";
...
function suffixPattern(conventions: NameConvention): RegExp | null {
  ...
  const alternatives = conventions.suffixes
    .map((suffix) =>
      Array.from(suffix.replace(/[^\p{L}\p{N}]/gu, ""))
...
function extractNamesFromText(
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    for (
      ...
      index >= 0 && nameWords.length < MENTION_MAX_NAME_WORDS;
      index -= 1
    ) {
...
export function buildLineContextMap(
  rows: DocumentLineRow[],
  radius = MENTION_CONTEXT_LINE_RADIUS,
  maxChars = MENTION_CONTEXT_MAX_CHARS,
): Map<string, string> {
...
export function extractMentionsFromLine(
  row: DocumentLineRow,
...
export function extractMentionsFromLines(
  rows: DocumentLineRow[],
  vocabulary: DomainVocabulary,
): RawDocumentMention[] {
  ...
  return rows.flatMap((row) =>
    extractMentionsFromLine(
      row,
      vocabulary,
      contextMap.get(lineContextKey(row.document_id, row.page, row.line)),
    ),
  );
}
...
export function toMaterializedMentions(
  ...
  return mentions.map((mention) => ({
    mentionId: mentionId(
      mention.documentId,
      mention.mentionType,
      mention.normalizedValue,
      mention.page,
      mention.line,
    ),
    documentId: mention.documentId,
    entityId:
      mention.mentionType === ENTITY_MENTION_TYPE
