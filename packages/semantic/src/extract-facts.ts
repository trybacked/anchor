...
const CURRENCY_MARK =
  "(?:[€$£¥₹₽₺¢]|\\b(?:EUR|USD|GBP|CHF|JPY|CNY|SEK|NOK|DKK|PLN|CAD|AUD)\\b|\\b[Ee]ur[oi]\\b)";
...
function classifyQuantity(
  ...
  const window = text
    .slice(Math.max(0, quantityEnd - FACT_CUE_WINDOW_CHARS), quantityEnd)
    .toLowerCase();
  let best: {
    factType: FactType;
    position: number;
...
export function extractFactsFromLine(
  row: DocumentLineRow,
  mentions: RawDocumentMention[],
  vocabulary: DomainVocabulary,
): RawFact[] {
  const nearby = buildNearbyMentions(mentions, row.document_id);
  const byQuantity = (quantity: FactType["quantity"]): FactType[] =>
    vocabulary.factTypes.filter((factType) => factType.quantity === quantity);
  return [
    ...collectQuantities(
      row,
      currencyPattern(vocabulary.numberFormat),
      byQuantity("currency"),
      vocabulary.numberFormat,
      nearby,
    ),
    ...collectQuantities(
      row,
      percentagePattern(vocabulary.numberFormat),
      byQuantity("percentage"),
      vocabulary.numberFormat,
      nearby,
    ),
  ];
}
export function extractFactsFromLines(
