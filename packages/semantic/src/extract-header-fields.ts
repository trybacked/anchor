...
import {
  ...
  HEADER_ALL_CAPS_MIN_LETTERS,
  HEADER_OCR_MIN_ALPHANUMERIC_RATIO,
} from "./constants.js";
...
export function isOcrNoiseLine(line: string): boolean {
  ...
  return (
    letters.length >= HEADER_ALL_CAPS_MIN_LETTERS &&
...
function extractIdentifierForFormat(
  text: string,
  format: DomainVocabulary["identifierFormats"][number],
): string | null {
  if (format.cues.length === 0) {
    return null;
  }
  const pattern = new RegExp(
    `\\b(?:${format.cues.map(escapeForRegex).join("|")})\\s*[:.]?\\s*(\\S+)`,
    "i",
  );
...
export function filterHeaderLinesForLlm(
  headerLines: string[],
  recurringLines: Set<string>,
): string[] {
...
export function findRecurringLines(
  headerLinesBySample: string[][],
  ratio = BOILERPLATE_DOCUMENT_RATIO,
  minSamples = BOILERPLATE_MIN_DOCUMENTS,
): Set<string> {
