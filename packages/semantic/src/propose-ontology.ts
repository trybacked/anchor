...
export function shouldSplitOntologyBurst(
  tables: CompressedTable[],
  documentCatalog?: DocumentCatalog,
): boolean {
  const documentTypeCount = documentCatalog?.documentTypes.length ?? 0;
  return (
    tables.length >= ONTOLOGY_SPLIT_TABLE_THRESHOLD ||
...
async function runSplitOntologyBurst(
  ...
  const relationsResult = await runBurst({
    model: models.language,
    system: ONTOLOGY_SYSTEM_PROMPT,
    prompt: ontologyRelationsPrompt(
      tables,
      classification,
      entitiesResult.output.entities,
      documentCatalog,
    ),
          ...
          doubts: [
            {
              ...
              reason:
                "Each line-document table is one extracted file (page/line/text). One entity per file was created deterministically; regroup by document type during review.",
            },
          ],
        },
        usage: EMPTY_BURST_USAGE,
