...
export function hashHeaderContent(
  headerLines: string[],
  recurringLines: Set<string> = new Set(),
): string {
  const payload = filterHeaderLinesForLlm(headerLines, recurringLines)
    .map(normalizeLine)
    .join("\n");
  return hashPayload(payload);
}

export function hashDocumentSample(
