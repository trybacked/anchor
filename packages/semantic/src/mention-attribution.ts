...
export function nearestMention<
  T extends {
    page: number;
    line: number;
  },
...
export function attributeIdentifierEntityId(
  mention: RawDocumentMention,
  entities: Map<string, EntityRecord>,
  entityMentions: RawDocumentMention[],
): string | null {
  const contextLower = mention.context.toLowerCase();
  const matchingEntities = [...entities.values()].filter((entity) =>
    contextLower.includes(entity.normalizedName.toLowerCase()),
  );
  ...
  const candidateMentions = entityMentions.filter(
    (entityMention) =>
      entityMention.documentId === mention.documentId &&
