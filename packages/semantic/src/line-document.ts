...
import {
  DOCUMENT_CORPUS_LINE_TABLE_RATIO,
  DOCUMENT_CORPUS_LINE_TABLE_THRESHOLD,
  LINE_DOCUMENT_COLUMN_CONFIDENCE,
  LINE_DOCUMENT_ENTITY_CONFIDENCE,
  PIPELINE_METADATA_COLUMN_CONFIDENCE,
  PIPELINE_METADATA_DEFAULT_CONFIDENCE,
} from "./constants.js";
  ...
  return {
    ...
    otherDocumentTables:
      otherDocumentTables.length > 20
        ? [
            ...otherDocumentTables.slice(0, 20),
            `…and ${String(otherDocumentTables.length - 20)} more`,
          ]
        : otherDocumentTables,
  };
}
