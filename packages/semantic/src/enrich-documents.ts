...
import {
  ...
  DOCUMENT_ENRICHMENT_MAX_SAMPLE_CHARS,
  DOCUMENT_ENRICHMENT_MAX_SUMMARY_CHARS,
  DOCUMENT_ENRICHMENT_MAX_TOPICS,
  DOCUMENT_ENRICHMENT_SAMPLE_LINE_LIMIT,
} from "./constants.js";
    ...
    documents: z.array(
      z.object({
        ...
        topics: (topicIds.length > 0
          ? z.array(z.enum(topicIds as [string, ...string[]]))
...
export function buildDocumentTopicSamples(rows: DocumentLineRow[]): Map<string, string> {
  ...
  return new Map(
    [...linesByDocument.entries()].map(([documentId, lines]) => [
      documentId,
...
function toEnrichmentInput(
  ...
  return {
    ...
    sample:
      sampleByDocument.get(document.sourceTable) ??
...
function applyEnrichment(
  documents: DocumentCatalogEntry[],
  enriched: Map<string, EnrichedDocument>,
...
export async function enrichDocuments(
  ...
  const inputs = options.documents.map((document) =>
    toEnrichmentInput(document, options.sampleByDocument),
  );
