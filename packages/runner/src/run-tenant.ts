...
function proposalToPersistedModel(
  proposal: Proposal,
  env?: Record<string, string | undefined>,
): SemanticModel {
...
async function persistPipelineArtifacts(
  ...
  try {
    const profile = existsSync(pipelineResult.profilePath)
      ? ProfileReportSchema.parse(
          JSON.parse(await readFile(pipelineResult.profilePath, "utf8")) as unknown,
        )
      : readRunArtifact(
          workspace.paths.workDir,
          pipelineResult.runId,
...
export async function runTenantPipeline(
  options: RunTenantPipelineOptions,
): Promise<RunTenantPipelineResult> {
  ...
  try {
    ...
    const existingModel =
      hasExistingModel && !options.forceFull
