...
async function embedDocumentChunks(
  ...
  if (chunkTexts.length === 0) {
    if (chunked.embeddingsRestored > 0) {
      progress.success(
        `Semantic search ready (${String(chunked.embeddingsRestored)} vectors preserved)`,
      );
    }
    return 0;
  }
...
async function resolveVocabulary(
  ...
  const sampleLines = await fetchCorpusSampleLines(
    session.query,
    lineDocuments.map((table) => table.table),
...
export async function runDocumentStage(
  ...
  const catalogCache = loadDocumentCatalogCache(
    root,
    previousRunId,
    forceFull,
  ...
  const typeRegistry =
    forceFull || persistedDocumentCatalog === undefined
      ? undefined
  ...
  const documentsToEnrich = materialized.catalog.documents.filter((document) =>
    newDocumentSourceTables.has(document.sourceTable),
  );
  ...
  const tableByDocumentType = new Map(
    materialized.catalog.documentTypes.map((type) => [type.id, type.tableName]),
  );
  await applyDocumentFields(
    session.query,
    enrichedDocumentEntries.flatMap((document) => {
  ...
  session.datasets = session.datasets.filter(
    (dataset) => !PIPELINE_DATASET_TABLES.has(dataset.tableName),
  );
  session.datasets.push(...materializedMentions.datasetsAdded);
  const materializedFacts = await materializeFacts(
    session.query,
    extractFactsFromLines(lineRows, rawMentions, vocabulary),
  );
