...
export function resolveNewDocumentSourceTables(
  ...
  for (const document of documents) {
    ...
    const known = persistedCatalog.documents.some(
      (entry) => entry.sourceTable === document.sourceTable,
    );
    if (!known) {
      newTables.add(document.sourceTable);
    }
...
export function mergeEnrichedDocuments(
  ...
  const enrichedBySource = new Map(
    enrichedDocuments.map((document) => [document.sourceTable, document]),
  );
