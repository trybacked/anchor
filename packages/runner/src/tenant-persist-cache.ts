...
export function mergeProfiles(
  existing: ProfileReport | undefined,
  incoming: ProfileReport,
): ProfileReport {
...
export function buildDocumentTypeRegistry(
  catalog: DocumentCatalog,
): Map<string, DocumentTypeRegistryEntry> {
  return new Map(
...
export function mergeDocumentCatalogs(
  ...
  const documentsBySource = new Map(
    existing.documents.map((document) => [document.sourceTable, document]),
  );
...
export async function loadTenantPersistedArtifacts(
  persistDir: string,
): Promise<TenantPersistedArtifacts> {
...
export async function persistTenantVocabulary(
  persistDir: string,
  vocabulary: DomainVocabulary,
): Promise<void> {
  await writeFile(
