...
export async function profileDataset(query: SqlQuery, dataset: Dataset): Promise<TableProfile> {
...
async function fetchTopValues(query: SqlQuery, table: string, column: string): Promise<TopValue[]> {
    const rows = await query(`SELECT ${column}::VARCHAR AS value, COUNT(*) AS count
     FROM ${table}
