...
function buildDocumentUnion(tables: string[]): string | null {
  ...
  return tables
    .map(
      (table) => `SELECT document_id, ${quoteString(table)} AS document_table
...
async function readAttributedIdentifiers(
  query: SqlQuery,
  entityId: string,
