...
export function formatEmbeddingLiteral(
  values: number[],
  dimension: number = DEFAULT_EMBEDDING_DIMENSION,
): string {
  if (values.length !== dimension) {
    throw new Error(
      `Embedding dimension ${String(values.length)} does not match expected ${String(dimension)}.`,
    );
  }
...
export async function ensureChunkEmbeddingColumn(query: SqlQuery): Promise<void> {
  const columns = await query(
    `SELECT column_name FROM information_schema.columns WHERE table_name = ${quoteString(DOCUMENT_CHUNKS_TABLE)} AND column_name = ${quoteString(CHUNK_EMBEDDING_COLUMN)}`,
  );
  if (columns.length === 0) {
    await query(
      `ALTER TABLE ${quoteIdentifier(DOCUMENT_CHUNKS_TABLE)} ADD COLUMN ${quoteIdentifier(CHUNK_EMBEDDING_COLUMN)} FLOAT[${String(DEFAULT_EMBEDDING_DIMENSION)}]`,
    );
  }
}
export async function capturePreservedChunkEmbeddings(
  ...
  try {
    ...
    const rows = await query(
      `SELECT ${quoteIdentifier("document_id")}, ${quoteIdentifier("text")}, ${quoteIdentifier(CHUNK_EMBEDDING_COLUMN)} FROM ${quoteIdentifier(DOCUMENT_CHUNKS_TABLE)} WHERE ${quoteIdentifier(CHUNK_EMBEDDING_COLUMN)} IS NOT NULL`,
    );
...
export async function restorePreservedChunkEmbeddings(
  ...
  await ensureChunkEmbeddingColumn(query);
  const rows = await query(
    `SELECT ${quoteIdentifier("document_id")}, ${quoteIdentifier("chunk_index")}, ${quoteIdentifier("text")} FROM ${quoteIdentifier(DOCUMENT_CHUNKS_TABLE)} WHERE ${quoteIdentifier(CHUNK_EMBEDDING_COLUMN)} IS NULL`,
  );
...
export async function fetchChunkTextsForEmbedding(query: SqlQuery): Promise<ChunkTextRow[]> {
  await ensureChunkEmbeddingColumn(query);
  const rows = await query(
    `SELECT ${quoteIdentifier("document_id")}, ${quoteIdentifier("chunk_index")}, ${quoteIdentifier("text")} FROM ${quoteIdentifier(DOCUMENT_CHUNKS_TABLE)} WHERE ${quoteIdentifier(CHUNK_EMBEDDING_COLUMN)} IS NULL ORDER BY ${quoteIdentifier("document_id")}, ${quoteIdentifier("chunk_index")}`,
  );
  return rows.map((row) => ({
...
export async function storeChunkEmbeddings(
  ...
  for (let offset = 0; offset < items.length; offset += CHUNK_EMBEDDING_STORE_BATCH_SIZE) {
    ...
    const values = batch
      .map(
        (item) =>
          `(${quoteString(item.document_id)}, ${String(item.chunk_index)}, ${formatEmbeddingLiteral(item.embedding)})`,
      )
      .join(", ");
    await query(
      `UPDATE ${quoteIdentifier(DOCUMENT_CHUNKS_TABLE)} AS c SET ${quoteIdentifier(CHUNK_EMBEDDING_COLUMN)} = v.${CHUNK_EMBEDDING_COLUMN} FROM (VALUES ${values}) AS v(document_id, chunk_index, ${CHUNK_EMBEDDING_COLUMN}) WHERE c.${quoteIdentifier("document_id")} = v.document_id AND c.${quoteIdentifier("chunk_index")} = v.chunk_index`,
    );
  }
}
export async function documentChunksHaveEmbeddings(query: SqlQuery): Promise<boolean> {
  try {
    await ensureChunkEmbeddingColumn(query);
    const rows = await query(
      `SELECT 1 FROM ${quoteIdentifier(DOCUMENT_CHUNKS_TABLE)} WHERE ${quoteIdentifier(CHUNK_EMBEDDING_COLUMN)} IS NOT NULL LIMIT 1`,
    );
