...
export async function registerLineTable(query: SqlQuery, tableName: string, rows: LineRow[]): Promise<void> {
    ...
    for (let offset = 0; offset < rows.length; offset += LINE_INSERT_BATCH_SIZE) {
        ...
        const values = batch
            .map((row) => `(${String(row.page)}, ${String(row.line)}, ${quoteString(row.text)})`)
            .join(", ");
        await query(`INSERT INTO ${quoteIdentifier(tableName)} VALUES ${values}`);
    }
}
export function linesFromPlainText(text: string, page = 1): LineRow[] {
    const rows: LineRow[] = [];
    let lineNum = 0;
    for (const raw of text.split(/\r?\n/u)) {
        ...
        lineNum += 1;
        rows.push({ page, line: lineNum, text: trimmed });
    }
    return rows;
}
