...
async function semanticSearch(
  ...
  try {
    ...
    throw new Error(
      `Query embedding dimension ${String(queryEmbedding.length)} does not match expected ${String(DEFAULT_EMBEDDING_DIMENSION)}. Re-run "backed model" with the configured embedding model.`,
    );
  }
