...
async function distinctValues(query: SqlQuery, table: string, column: string): Promise<string[]> {
  const rows =
    await query(`SELECT DISTINCT ${quoteIdentifier(column)} AS value FROM ${quoteIdentifier(table)}
...
function buildFactColumns(
  presentFactTypes: string[],
  factTypes: MaterializeEntityProfilesOptions["factTypes"],
): FactColumn[] {
  ...
  return presentFactTypes.map((factType) => {
    ...
    const alias = quoteIdentifier(
      `${AGGREGATION_PREFIXES[aggregation]}_${sqlColumnSuffix(factType)}`,
    );
    return {
      alias,
...
const DOCUMENT_IDENTIFIER_ROLLUP: IdentifierRollupConfig = {
  ...
  subquery: (
    mentions,
    identifierType,
  ) => `SELECT DISTINCT docs.entity_id, coded.normalized_value AS value
...
function buildIdentifierRollups(
  identifierTypes: string[],
  config: IdentifierRollupConfig,
): {
  ctes: string[];
  selects: string[];
  joins: string[];
...
function buildProfilesQuery(
  ...
  identifiers: ReturnType<typeof buildIdentifierRollups>,
  attributedIdentifiers: ReturnType<typeof buildIdentifierRollups>,
): string {
...
export async function materializeEntityProfiles(
  query: SqlQuery,
  options: MaterializeEntityProfilesOptions,
): Promise<MaterializeEntityProfilesResult> {
  const presentFactTypes = await distinctValues(query, DOCUMENT_FACTS_TABLE, "fact_type");
  const identifierTypes = (
    await distinctValues(query, DOCUMENT_MENTIONS_TABLE, "mention_type")
  ).filter((mentionType) => mentionType !== ENTITY_MENTION_TYPE);
  await dropTableIfExists(query, ENTITY_PROFILES_TABLE);
  await query(
    buildProfilesQuery(
      await findTopicTables(query),
      buildFactColumns(presentFactTypes, options.factTypes),
      buildIdentifierRollups(identifierTypes, DOCUMENT_IDENTIFIER_ROLLUP),
      buildIdentifierRollups(identifierTypes, ATTRIBUTED_IDENTIFIER_ROLLUP),
    ),
  );
  const countRows = await query(
    `SELECT COUNT(*) AS profile_count FROM ${quoteIdentifier(ENTITY_PROFILES_TABLE)}`,
  );
