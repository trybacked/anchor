...
function resolveEntityForDocumentId(
  documentId: string,
  model: SemanticModel,
  sourceTableIndex: Map<string, string>,
): Entity | undefined {
...
export async function searchModelViaDocumentChunks(
  model: SemanticModel,
  chunkSearch: ChunkSearcher,
  query: string,
  catalog: DocumentCatalog | undefined,
  limit: number = DEFAULT_CHUNK_SEARCH_LIMIT,
  ...
  const sourceTableIndex =
    catalog !== undefined ? buildDocumentSourceTableIndex(catalog) : new Map<string, string>();
