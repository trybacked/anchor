...
async function createDocumentMentionsTable(
  query: SqlQuery,
  mentions: MaterializedMentionInput[],
): Promise<void> {
...
export async function fetchAllDocumentLines(query: SqlQuery): Promise<
  Array<{
    document_id: string;
    page: number;
    line: number;
...
> {
  const rows = await query(
    `SELECT document_id, page, line, text FROM ${quoteIdentifier(DOCUMENT_LINES_TABLE)} ORDER BY document_id, page, line`,
  );
  return rows.map((row) => ({
...
export async function materializeDocumentMentions(
  query: SqlQuery,
  input: MaterializeDocumentMentionsInput,
): Promise<MaterializeDocumentMentionsResult> {
