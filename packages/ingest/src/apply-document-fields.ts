...
async function ensureDocumentFieldColumns(
  query: SqlQuery,
  tableName: string,
  fieldKeys: string[],
): Promise<void> {
  ...
  for (const key of fieldKeys) {
    if (!existing.has(key)) {
      await query(
        `ALTER TABLE ${quoteIdentifier(tableName)} ADD COLUMN ${quoteIdentifier(key)} VARCHAR`,
      );
    }
  }
}

export async function applyDocumentFields(
  query: SqlQuery,
  ...
  for (const update of updates) {
    ...
    await ensureDocumentFieldColumns(
      query,
      update.tableName,
      fieldEntries.map(([key]) => key),
    );
    const assignments = fieldEntries.map(
      ([key, value]) => `${quoteIdentifier(key)} = ${sqlNullableString(value ?? null)}`,
    );
