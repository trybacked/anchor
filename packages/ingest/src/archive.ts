...
async function loadUnrarWasm(): Promise<ArrayBuffer> {
    if (unrarWasmBinary === null) {
        ...
        unrarWasmBinary = file.buffer.slice(file.byteOffset, file.byteOffset + file.byteLength);
    }
...
export async function extractZipArchive(absolutePath: string, targetDir: string): Promise<void> {
...
export async function extractRarArchive(absolutePath: string, targetDir: string): Promise<void> {
    await mkdir(targetDir, { recursive: true });
