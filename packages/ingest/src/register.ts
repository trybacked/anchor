...
/**
 * Register a scanned source file as a DuckDB view/table for downstream pipeline stages.
 * Dispatches by `source.format` and returns dataset metadata plus any ingest warnings.
 */
...
async function registerCsv(
  ...
  if (encoding === "latin-1") {
    warnings.push(nonUtf8EncodingWarning(source.relativePath));
  }
  ...
  await query(`CREATE VIEW ${quoteIdentifier(tableName)} AS ${sniffed.readCsvClause}`);
  return {
    dataset: {
      tableName,
  ...
  if (rows.length === 0) {
    ...
    if (rows.length > 0) {
      ...
      warnings.push({
        ...
        file: source.relativePath,
        message: "Scanned PDF: OCR skipped — install Poppler (pdftoppm), e.g. brew install poppler",
      });
    }
  }
...
function nonUtf8EncodingWarning(relativePath: string): IngestWarning {
  return {
    ...
    file: relativePath,
    message: "Non-UTF-8 encoding detected (likely Windows-1252): read as Latin-1",
  };
}
