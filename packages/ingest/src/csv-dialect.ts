...
import { ENCODING_PROBE_MAX_BYTES, UTF8_TAIL_TRIM_BYTES } from "./constants.js";
...
export async function detectEncoding(filePath: string): Promise<CsvEncoding> {
  ...
  try {
    ...
    const probe = size > probeSize ? buffer.subarray(0, probeSize - UTF8_TAIL_TRIM_BYTES) : buffer;
    try {
...
async function sniffCsv(
  ...
  const fractionalColumnCount = columns.filter(
    (column: unknown) =>
      ...
      column !== null &&
      NATIVE_FRACTIONAL_TYPE_PATTERN.test(
        String(
          (
            column as {
              type?: unknown;
