/** Domain vocabulary schemas and merge helpers for tenant-specific terminology. */
import { z } from "zod";
...
export const DomainTermSchema = z.object({
  id: z.string().min(1).regex(SLUG_ID_PATTERN, "Term ids are lowercase slugs"),
  label: z.string().min(1),
  description: z.string().min(1),
});
export const FactTypeSchema = z.object({
  id: z.string().min(1).regex(SLUG_ID_PATTERN, "Fact type ids are lowercase slugs"),
  label: z.string().min(1),
...
export const IdentifierFormatSchema = z.object({
  id: z.string().min(1).regex(SLUG_ID_PATTERN, "Identifier ids are lowercase slugs"),
  label: z.string().min(1),
  cues: z.array(z.string().min(1)).min(1),
...
export function mergeVocabulary(
  discovered: DomainVocabulary,
  overrides: DomainVocabularyOverrides | undefined,
): DomainVocabulary {
