 ...
 *
 */
export type ObjectQueryExecutor = (
 ...
 *
 */
export type ObjectQueryReader = (
  request: TableObjectQueryRequest,
) => Promise<Record<string, unknown>[]>;
/** Resolves the output column alias for an aggregation operation. */
export function resolveAggregationAlias(aggregation: AggregateOp, index: number): string {
  ...
  return index === 0
    ? DEFAULT_COUNT_AGGREGATION_ALIAS
    : `${DEFAULT_COUNT_AGGREGATION_ALIAS}_${String(index)}`;
}
