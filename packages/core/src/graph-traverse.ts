...
/** Clamps graph traversal depth to {@link MIN_TRAVERSE_DEPTH}..{@link MAX_TRAVERSE_DEPTH}. */
...
function findNextRelation(
  ...
  direction: TraverseDirection,
  usedRelationIds: ReadonlySet<string>,
): Relation | null {
...
/** Builds a relation hop path from a starting relation, or `null` when the relation is missing. */
...
/** Resolves hop metadata to full relation segments, or `null` when a hop is invalid. */
export function resolveRelationPath(
  model: SemanticModel,
  hops: RelationHop[],
): RelationPathSegment[] | null {
