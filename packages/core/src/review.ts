...
function applyVerdicts<T extends Entity | Relation | Rule>(
  elements: T[],
  kind: ReviewQuestionKind,
  verdicts: Map<string, ElementVerdict>,
  reviewedTargetKeys: Set<string>,
  reviewConfidenceThreshold: number,
  shouldKeep: (element: T) => boolean = () => true,
): T[] {
  return elements
    .map((element) =>
      applyVerdict(
        element,
        verdicts.get(verdictKey(kind, element.id)),
        reviewedTargetKeys.has(verdictKey(kind, element.id)),
...
export function applyReview(
  ...
  const relations = applyVerdicts(
    proposal.relations,
    "relation",
    verdicts,
    reviewedTargetKeys,
    reviewConfidenceThreshold,
    (relation) => keptEntityIds.has(relation.fromEntity) && keptEntityIds.has(relation.toEntity),
  );
  const rules = applyVerdicts(
    proposal.rules,
    "rule",
    verdicts,
    reviewedTargetKeys,
