...
- Review question types live in `review-questions.ts`; proposal/review answer types are split across `proposal.ts` and `review.ts`
- Workspace layout: `workspacePaths`, `createRunId`, `DEFAULT_WORKSPACE_CONFIG`, validated read/write of config and artifacts (`.backed/`, `.backed/runs/<id>/`), YAML serialization of `model.yaml`
- Shared constants: `LOW_CONFIDENCE_THRESHOLD`, `DEFAULT_REVIEW_CONFIDENCE_THRESHOLD`, `MODEL_FORMAT_VERSION`

**Main public API:** schemas + inferred types, `applyReview`, `writeWorkspaceConfig` / `readWorkspaceConfig`, `writeRunArtifact` / `readRunArtifact` / `listRunIds`, `writeModelYaml` / `readModelYaml` / `serializeModelYaml` / `parseModelYaml`.
