...
export function printPipelineSummary(
  ui: Ui,
  result: RunAnchorPipelineResult,
  skipEmbed: boolean,
): void {
  ...
  if (proposal.usage) {
    const cost =
      proposal.usage.costUsd !== null
        ? ` ${ui.dim(`(~$${proposal.usage.costUsd.toFixed(4)})`)}`
        : "";
    ui.log(
  ...
  ui.step(
    `${String(proposal.questions.length)} review question(s) — run ${ui.command(formatCliCommand(COMMANDS.REVIEW))}`,
  );
  ui.blank();
  const parts = [
