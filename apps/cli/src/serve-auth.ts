...
async function assertGatewayReachable(apiUrl: string): Promise<void> {
  ...
  try {
    ...
    throw new ServeAuthError(
      `Backed gateway unreachable at ${apiUrl}. Check your network connection and try again.`,
    );
  }
  if (!response.ok) {
    throw new ServeAuthError(
      `Backed gateway unreachable at ${apiUrl} (HTTP ${String(response.status)}).`,
    );
  }
}
...
async function buildTelemetryRecorder(
  ...
  try {
    ...
    if (error instanceof BackedApiError && error.status === 401) {
      throw new ServeAuthError(
        `Backed session expired or invalid. Run ${formatCliCommand(COMMANDS.LOGIN)} again.`,
      );
    }
