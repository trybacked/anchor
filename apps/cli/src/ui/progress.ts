...
export function createTaskProgress(initialLabel: string, total?: number): ProgressHandle {
  ...
  const render = (): void => {
    ...
    const eta =
      resolvedTotal > 0 ? estimateRemainingSeconds(completed, resolvedTotal, startedAtMs) : null;
    const suffix =
      resolvedTotal > 0
        ? ` ${renderBar(completed, resolvedTotal, 16)} ${String(completed)}/${String(resolvedTotal)}${eta !== null ? paint(ANSI.dim, ` ~${formatEtaSeconds(eta)} left`) : ""}`
        : "";
