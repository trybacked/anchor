...
export function printDocumentTypeHintGuide(ui: Ui): void {
  ...
  ui.blank();
  ui.log(
    ui.box("Document filename rules (optional)", [
...
async function promptKeywordForType(
  ...
  ui.detail(
    `  Example: "${demoFilename}" → slug ${demoSlug} → ${trimmed.length > 0 && demoSlug.includes(trimmed.toLowerCase()) ? "matches" : "check your spelling"}`,
  );
  return trimmed;
}
async function promptDocumentTypeGroup(
  theme: ReturnType<typeof createPromptTheme>,
  ui: Ui,
...
async function collectDocumentTypeHints(
  theme: ReturnType<typeof createPromptTheme>,
  ui: Ui,
  initial: DocumentTypeHintConfig[] = [],
): Promise<DocumentTypeHintConfig[]> {
  const hints = [...initial];
  if (hints.length === 0) {
    const start = await confirm({
      message:
        "Add document filename rules? (recommended for PDF folders — skips LLM when filenames match)",
      default: false,
      theme,
    });
    if (!start) {
...
export async function promptDocumentTypeHints(
  ...
  if (hints.length > 0) {
    ui.detail(
      `  ${String(hints.length)} rule(s) already in config (e.g. match "${hints[0]?.match ?? ""}" → ${hints[0]?.documentTypeLabel ?? ""}).`,
    );
