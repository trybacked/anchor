...
function printGatewayHelp(): void {
  const ui = getUi();
  ui.log(
    `${ui.label("Usage:")} ${ui.command(formatCliCommand(COMMANDS.GATEWAY))} ${ui.dim("[options]")}`,
  );
  ui.blank();
  ui.log(ui.label("Options:"));
  ui.log(
    `  ${ui.command(FLAGS.KEY)}, ${ui.command(FLAGS.KEY_SHORT)} ${ui.dim("<key>")}   Set key without a prompt (CI / scripts)`,
  );
