...
function printLoginHelp(): void {
  const ui = getUi();
  ui.log(
    `${ui.label("Usage:")} ${ui.command(formatCliCommand(COMMANDS.LOGIN))} ${ui.dim("[options]")}`,
  );
  ui.blank();
  ui.log(ui.label("Options:"));
  ui.log(`  ${ui.command(FLAGS.STATUS)}              Show the signed-in Backed account`);
  ui.log(
    `  ${ui.command(FLAGS.TOKEN)} ${ui.dim("<token>")}       Save a personal access token (non-interactive)`,
  );
...
export const loginCommand: CommandHandler = async (args) => {
  ...
  try {
    ...
    if (wantsHeadlessCommand()) {
      throw new Error(
        `Interactive login requires a terminal. Use ${FLAGS.TOKEN} for non-interactive auth.`,
      );
    }
