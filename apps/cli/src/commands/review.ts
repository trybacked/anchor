...
function renderEvidenceTable(ui: ReturnType<typeof getUi>, evidence: EvidenceTable): string {
    ...
    const widths = evidence.columns.map((_, columnIndex) => Math.max(...rows.map((row) => (row[columnIndex] ?? "").length)));
    const renderRow = (row: string[], header = false): string => row
        .map((cell, index) => {
...
async function askQuestion(proposal: Proposal, questionIndex: number, theme: ReturnType<typeof createPromptTheme>): Promise<ReviewAnswer | null> {
    const ui = getUi();
    ...
    ui.blank();
    ui.hr();
    ui.log(`${ui.label(`Question ${String(questionIndex + 1)}/${String(proposal.questions.length)}`)} ${ui.dim(`· risk ${question.risk.toFixed(2)}`)}`);
...
function applyDocumentTypeRenamesFromReview(root: string, proposal: Proposal, review: Review, documentCatalog: DocumentCatalog | undefined): void {
    if (documentCatalog === undefined) {
        return;
    }
    const ui = getUi();
    const typeIdByEntityId = new Map(documentCatalog.documentTypes.map((type) => [slugifyDocumentTypeId(type.id), type.id]));
