...
function parsePartnersJson(raw: string | undefined): PartnerConfig[] {
  ...
  return parsed.map((entry) => {
    ...
    if (webhooksRaw !== undefined) {
      ...
      webhooks = {
        ...(typeof runCompleted === "string" && runCompleted.length > 0 ? { runCompleted } : {}),
      };
    }
    return {
      partnerId,
...
function parseControlPlaneConfig(
...
export function loadWorkerServiceConfig(
  ...
  return {
    ...
    rateLimitMaxRequests: readPositiveInt(
      env[ENV.RATE_LIMIT_MAX_REQUESTS],
      DEFAULT_RATE_LIMIT_MAX_REQUESTS,
    ),
